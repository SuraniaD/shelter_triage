...
if run_btn:
    ...
    if errors:
        ...
        with st.spinner("🤖 Generating triage report via llama3.1:8b..."):
        ...
        if error:
            ...
            st.session_state["intake_code"] = gen_intake_code()  # Ready for next intake
            st.rerun()
...
with report_col:
    ...
    if not st.session_state["last_report"]:
        ...
        if not report:
            ...
            st.divider()
            st.caption(
                f"🤖 Generated by {report.get('model_used', 'llama3.1:8b')} · "
